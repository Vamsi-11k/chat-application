...
export const Sidebar = ({
  ...
  const [activeNavTab, setActiveNavTab] = useState('chats'); // 'chats' | 'starred' | 'requests'
  ...
  const onlineFriends = friends.filter((f) => isUserOnline(f._id) || f.isOnline);
  const pendingIncomingCount = incomingRequests.length;
  const starredCount = starredMessages.length;

  return (
    <div className="h-full flex flex-row flex-shrink-0 z-20">
      
      {/* ===================================================================== */}
      {/* 1. FAR-LEFT ICON RAIL (Column 1 - Slim Vertical Strip)                */}
      {/* ===================================================================== */}
      <nav className="w-16 sm:w-18 h-full flex flex-col items-center justify-between py-5 bg-white/80 dark:bg-[#031512]/80 backdrop-blur-xl border-r border-teal-100/60 dark:border-[#092b26]/70 z-20 flex-shrink-0 transition-colors duration-200">
        ...
        <div className="flex flex-col items-center space-y-6">
          ...
          <div className="flex flex-col items-center space-y-3">
            ...
            {/* Starred Messages Navigation Item */}
            <button
              ...
              className={`relative w-11 h-11 rounded-2xl flex items-center justify-center transition-all duration-200 ${
                activeNavTab === 'starred'
                  ? 'bg-teal-600 text-white shadow-md shadow-teal-600/30'
                  : 'text-slate-400 hover:text-amber-500 dark:hover:text-amber-400 hover:bg-amber-50/70 dark:hover:bg-[#092a25]'
              }`}
            >
              <Star size={19} className={activeNavTab === 'starred' ? 'fill-white' : ''} />
              {starredCount > 0 && (
                <span className="absolute -top-1 -right-1 px-1.5 min-w-[18px] h-4.5 bg-amber-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center border-2 border-white dark:border-[#031512] shadow-xs">
                  {starredCount}
                </span>
              )}
            </button>
        ...
        <div className="flex flex-col items-center space-y-3.5 relative">
          <ThemeToggle
            direction="top"
            align="left"
            className="w-10 h-10 rounded-2xl bg-teal-50/80 dark:bg-[#092a25] border border-teal-100 dark:border-[#103a33] text-teal-600 dark:text-teal-300 shadow-xs"
          />
              ...
              <>
                ...
                <div className="absolute left-full bottom-0 ml-3 w-52 bg-white dark:bg-[#061e1a] border border-teal-100 dark:border-[#0f3d37] rounded-2xl shadow-2xl shadow-black/50 py-2 z-50 animate-fadeIn text-xs">
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2.5 text-left font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-[#123832] flex items-center justify-between transition-colors border-t border-slate-100 dark:border-[#0f3d37]/80"
                  >
                    <div className="flex items-center space-x-2.5">
                      <Wand2 size={14} className={effectsEnabled ? 'text-teal-500 animate-pulse' : 'text-slate-400'} />
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2.5 text-left font-medium text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-[#123832] flex items-center space-x-2.5 transition-colors border-t border-slate-100 dark:border-[#0f3d37]/80"
                  >
                    <Trash2 size={14} />
      ...
      {/* ===================================================================== */}
      <aside className="w-full sm:w-80 md:w-84 lg:w-92 h-full flex flex-col bg-slate-50/75 dark:bg-[#051c18]/75 backdrop-blur-xl border-r border-teal-100/60 dark:border-[#0c3530]/70 flex-shrink-0 transition-colors duration-200">
        
        {/* Chats Tab Header & Body */}
        {activeNavTab === 'chats' && (
          <>
                  ...
                  <div className="py-16 px-4 text-center text-slate-400 space-y-3">
                    ...
                    <div>
                      <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                        {friends.length === 0 ? 'No Contacts Yet' : 'No Results Found'}
                      </h4>
